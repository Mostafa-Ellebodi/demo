...
def upload_s3(fName):
    cmd0 = 'aws s3 rm s3://tcsysdev-ml-1 --recursive'
    os.system(cmd0)
    print("command one above")
    cmd1 = 'aws s3 cp ./Results/{0} s3://tcsysdev-ml-1/{1}'.format(fName,fName)
    os.system(cmd1)
    print("command two above")
    cmd2 = 'aws s3api put-object-acl --bucket tcsysdev-ml-1 --key {0} --acl public-read'.format(fName)
    os.system(cmd2)
