...
def upload_s3(fName):
    cmd0 = 'aws s3 rm s3://streamlit-toc-docx --recursive'
    os.system(cmd0)
    print("command one above")
    cmd1 = 'aws s3 cp ./Results/{0} s3://streamlit-toc-docx/{1}'.format(fName,fName)
    os.system(cmd1)
    print("command two above")
    cmd2 = 'aws s3api put-object-acl --bucket streamlit-toc-docx --key {0} --acl public-read'.format(fName)
    os.system(cmd2)
    print("command three above")
...
def process_file(TEXT_FILE_PATH, 
    ...
    file_name = os.path.split(TEXT_FILE_PATH)[-1].split(".")[0]
    file_name_new = re.sub(r'\W+', '', file_name)
    mydoc.save(os.path.join(OUTPUT_FILE_PATH, file_name_new + ".docx"))
...
st.write("""
  # Get the best TOC for your Video!
""")
# st.subheader("upload your text file and look at TOC")



st.write("Copy and paste your text, and pick values from the slide bars on the left. Then, Hit Get TOC")
# uploaded_files  = st.file_uploader("Please upload your text file: ", type ="txt", accept_multiple_files=True)
inputText = st.text_area("Enter you text here", "")

# if inputText=='':
#   st.write("Please put some text")
#   st.stop()
...
#### Configs ####
#################
INITIAL_SEGMENTATION_LENGTH = st.sidebar.slider('Inital segmentation length (Recommended: 50)', 10, 80, 50)
RECURSIVE_SEGMENTATION_FACTOR = st.sidebar.slider('Recursive Segmentation Factor. (Recommended: 2)', 1, 5, 2)
STOPPING_LENGTH = st.sidebar.slider('Stopping Length. (Recommended: 50)', 10, 80, 50)
_RELATIVE = st.sidebar.selectbox(
    'Do you want the segmentatio length to be realtive? (Recommended: False)',
    ('False', 'True')
)
if _RELATIVE == 'True':
  RELATIVE = True
...
class model_args:
    ...
    model_name_or_path = '../pegasus_b8_lr-5_NoEarly_youtube_5/checkpoint-2606'
...
if st.button("Get TOC"):
    ...
    #     docs_headlines.append(headlines)
        ...
        fName = os.listdir(OUTPUT_PATH)[0]
        upload_s3(fName)
        link = "https://streamlit-toc-docx.s3.us-east-1.amazonaws.com/"+fName
        st.markdown(f"[Download {fName}]({link})", unsafe_allow_html = True)

        ## print TOC
        st.markdown('## Generated TOC..')
        for headline in headlines:
            st.markdown(f"***{headline}***")

        # else:
        #     cmd = 'zip -r Results/TOC_Results Results/'
        #     os.system(cmd)

        #     # shutil.make_archive(OUTPUT_PATH + 'Results', 'zip', OUTPUT_PATH)
        #     fName = 'TOC_Results.zip'
        #     upload_s3(fName)
        #     link = "https://tcsysdev-ml-1.s3.us-east-1.amazonaws.com/"+fName
        #     st.markdown(f"[Download {fName}]({link})", unsafe_allow_html = True)
        ...
        file = None
        if not file:
            st.write("Process finished. You can enter new text and hit Get Toc")
            # st.stop()
        print("end cleaning")
    except:
